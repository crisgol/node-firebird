...
SQLVarText.prototype.decode = function(data) {
    var ret;
    if (this.subType > 1) {
        ret = data.readText(this.length, DEFAULT_ENCODING);  //remove rigth space
    } else {
...
exports.Connection.prototype._bind_events = function(host, port, callback) {
    ...
    self._socket.on('data', function(data) {
        ...
        while (xdr.pos < xdr.buffer.length) {
            ...
            try {
                cb = self._queue[0];
                obj = decodeResponse(xdr, cb, self.db, self._lowercase_keys,self._trim_rigth);
                if (obj && obj.error) {
...
function decodeResponse(data, callback, db, lowercase_keys,trim_rigth){
    do {
                ...
                for (length = output.length; data.fcolumn < length; data.fcolumn++) {
                    ...
                    try {
                        ...
                        if (custom.asObject) {
                            ...
                            data.frow[data.fcols[data.fcolumn]] =  ((item.type === SQL_TEXT) && (trim_rigth))?value.trim():value;
                        }
                        else {
                            if (item.type === SQL_BLOB)
...
Connection.prototype.attach = function (options, callback, db) {
    ...
    this._trim_rigth = options.trim_rigth || false;
